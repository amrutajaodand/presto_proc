...
A.Jaodand, 05/12/2014
Last modification:11 Dec 2014
Modification: numout in prepsubband divided by downsampling factor  
...
os.system("ls %s"%os.path.join(path,"%s%s.fits"%(filename[:-1],'[1-3]')))
infilepath = os.path.join(path,"%s%s.fits"%(filename[:-1],'[1-3]'))
print infilepath	

# folder where the script is located 
src_dir = "/lustre/pulsar/scratch/ajaodand/GBT14B-036_AMXPs/scripts/presto_proc_15min"

##Initialise the files to store the results of the processing 
#create folder for a given date
outpath= "/lustre/pulsar/scratch/ajaodand/GBT14B-036_AMXPs/results_15min_11122014"
...
results_dir = os.path.join(outpath,path[49:],key)
...
print "ls %s/*.fft | xargs -n 1 accelsearch -zmax 100 > %s/accelsearch_data_%s"%(results_dir,results_dir,key)
os.system("ls %s/*.fft | xargs -n 1 accelsearch -zmax 100 > %s/accelsearch_data_%s"%(results_dir,results_dir,key))
