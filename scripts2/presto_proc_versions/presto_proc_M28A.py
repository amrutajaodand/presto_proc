...
key =filename # "GBT"  #used for naming all the files
key2 = "%s_out"%filename #"GBT_prep" #used for naming the .dat files from the prepsubband 
print key2 
...
os.system("single_pulse_search.py *%s*.*dat > %s_single_pulse_search_out"%(key,key))
...
for i in range(1,len(array)):
	...
	#now as the number of channels are 2048 everywher taking nsub = 64 and number of bins = 64 for all candidates doing the prepfold for whole data for all given candidates. 
		...
		os.system("prepfold -noxwin -n 64 -nsub 64 -p %0.17f -dm %3.2f %s > prepfold_rawdata_out"%(p_bary,float(array[i][1]),infilepath)) 
# 
os.chdir("%s"%results_dir)
os.system("ls *.ps | xargs -n 1 ps2pdf")
os.system(" mv *.pfd.pdf* prepfold_results/")
os.system("mv %s/*%s_* %s/prepfold_raw_data"%(path,filename,results_dir))  
