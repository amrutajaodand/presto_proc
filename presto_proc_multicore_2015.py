...
if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_%s"%key)):
	os.mkdir(os.path.join(results_dir,"prepfold_ephmer_2015_%s"%key))
...
os.system("mv *%s*topo* %s"%(key,results_dir))
...
def prepfold_ephmer_parallel_proc(i):
	if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f.tar.gz"%(key,key,float(array[i][1])))):   
		print "the path in question is %s"%(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f.tar.gz"%(key,key,float(array[i][1]))))
		if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f"%(key,key,float(array[i][1])))):
			os.mkdir(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f"%(key,key,float(array[i][1])))) #folder -store every .dat (DM)'s results folding it using all ephimeridis  
		print "changing path to folder %s"%(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f"%(key,key,float(array[i][1]))))
		os.chdir(os.path.join(results_dir,"prepfold_ephmer_2015_%s/prepfold_ephmer_2015_%s_DM%3.2f"%(key,key,float(array[i][1])))) #changing path to above initialised folder 
		...
		for p in range(int(-N*10),int(N*10),int(dt*10)):  #multiplication by 10^(-(power of 10 in the dt)), for loop does not accept the decimal values for stepsizes
			print "prepfold -noxwin -par %s/ephimeridis_2015_%s/SAXJ1808.4-3658_2015_%d.par ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_2015_%d > prepfold_ephmer_2015_out"%(path,filename,p,key2,float(array[i][1]),key2,float(array[i][1]),p)
			os.system("prepfold -noxwin -par %s/ephimeridis_2015_%s/SAXJ1808.4-3658_2015_%d.par ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_2015_%d > prepfold_ephmer_2015_out"%(path,filename,p,key2,float(array[i][1]),key2,float(array[i][1]),p))
			os.system("rm *.pfd")
			#os.system("ls *.ps | xargs -n 1 ps2pdf") 
		os.chdir("../")
		os.getcwd()
		os.system("tar -zcvf prepfold_ephmer_2015_%s_DM%3.2f.tar.gz prepfold_ephmer_2015_%s_DM%3.2f"%(key,float(array[i][1]),key,float(array[i][1])))
		os.system("rm -r prepfold_ephmer_2015_%s_DM%3.2f"%(key,float(array[i][1])))
#######################################################################################
start_time = time.time()
#
if N_cores > Ncore_max: # safety check
...
print index
pool = mp.Pool(processes=N_cores)                                       # specifies number of parallel processes
pool.map(prepfold_ephmer_parallel_proc,index)                                     # index sequence passed to pool for reference to datapoint at that index 
pool.close()
end_time = time.time()                                                  # time at which maximization ends
print 'time taken =%f s'%(end_time-start_time)
print "current directory is",(os.getcwd())
