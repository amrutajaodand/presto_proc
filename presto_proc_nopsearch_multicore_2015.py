...
try:
   opts, args = getopt.getopt(args,"h",["folder_tag=","file_tag=","N_cores=","f_num"])#, "m=","incl_angle="]) #folder_tag name of the folder
...
if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s"%key)):
	os.mkdir(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s"%key))
...
dir_ephmer = "ephimeridis_2015_%s"%(key)
...
f_ephmer="%s/%s_2015.par"%(path,infilename[12:-15]) 
...
for p in range(-N*10,N*10,int(dt*10)):
     ...
     fout = open("%s/SAXJ1808.4-3658_2015_TASC_%d.par"%(os.path.join(path,dir_ephmer),p),"w+")
     for line in fin:
        fout.write(line.replace('TASC            %f'%mjd_current,'TASC            %f'%mjd))
     fout.close()
     fin.close()
     fin = open("%s/SAXJ1808.4-3658_2015_TASC_%d.par"%(os.path.join(path,dir_ephmer),p),"rw")
     fout = open("%s/SAXJ1808.4-3658_2015_PEPOCH_%d.par"%(os.path.join(path,dir_ephmer),p),"wa+")
     for line in fin:
        fout.write(line.replace('PEPOCH          %f'%mjd_current,'PEPOCH          %f'%mjd))
     fout.close()
     fin.close()
     fin = open("%s/SAXJ1808.4-3658_2015_PEPOCH_%d.par"%(os.path.join(path,dir_ephmer),p),"rw")
     fout = open("%s/SAXJ1808.4-3658_2015_%d.par"%(os.path.join(path,dir_ephmer),p),"wa+")
     for line in fin:
...
def prepfold_ephmer_nopsearch_parallel_proc(i):
        if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f.tar.gz"%(key,key,float(array[i][1])))): 
		print "this does not exists %s"%(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f.tar.gz"%(key,key,float(array[i][1]))))
		if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,key,float(array[i][1])))):
			os.mkdir(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,key,float(array[i][1])))) #folder -store every .dat (DM)'s results folding it using all ephimeridis  
		print "changing path to folder %s"%(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,key,float(array[i][1]))))
		os.chdir(os.path.join(results_dir,"prepfold_ephmer_2015_nopsearch_%s/prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,key,float(array[i][1])))) #changing path to above initialised folder 
		...
		for p in range(int(-N*10),int(N*10),int(dt*10)):  #multiplication by 10^(-(power of 10 in the dt)), for loop does not accept the decimal values for stepsizes
			print "prepfold -noxwin -par %s/ephimeridis_2015_%s/SAXJ1808.4-3658_2015_%d.par -fine -nopsearch ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_2015_%d > prepfold_ephmer_2015_nopsearch_out"%(path,filename,p,key2,float(array[i][1]),key2,float(array[i][1]),p)

			os.system("prepfold -noxwin -par %s/ephimeridis_2015_%s/SAXJ1808.4-3658_2015_%d.par -fine -nopsearch ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_2015_%d > prepfold_ephmer_2015_nopsearch_out"%(path,filename,p,key2,float(array[i][1]),key2,float(array[i][1]),p))
			os.system("rm *.pfd")
			#os.system("ls *.ps | xargs -n 1 ps2pdf") 
		os.chdir("../")
		os.getcwd()
		os.system("tar -zcvf prepfold_ephmer_2015_nopsearch_%s_DM%3.2f.tar.gz prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,float(array[i][1]),key,float(array[i][1])))
		os.system("rm -r prepfold_ephmer_2015_nopsearch_%s_DM%3.2f"%(key,float(array[i][1])))
