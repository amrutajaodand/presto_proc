...
print os.getcwd()
print results_dir, key
dm_list = glob("%s/*%s_out*.dat"%(results_dir,key))
dm_vals = np.array([float(re.search('DM(.+?).dat', i).group(1)) for i in dm_list])
print dm_vals, "\n", len(dm_vals)

#####function to parallelize the prepfold with ephemeridis files#####################
def prepfold_ephmer_parallel_proc(i):
	
	if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_%s/prepfold_ephmer_%s_DM%3.2f.tar.gz"%(key,key,dm_vals[i]))):   
		if not os.path.exists(os.path.join(results_dir,"prepfold_ephmer_%s/prepfold_ephmer_%s_DM%3.2f"%(key,key,dm_vals[i]))):
			os.mkdir(os.path.join(results_dir,"prepfold_ephmer_%s/prepfold_ephmer_%s_DM%3.2f"%(key,key,dm_vals[i]))) #folder -store every .dat (DM)'s results folding it using all ephimeridis  
		print "reached inside the execution loop"
		print "changing path to folder %s"%(os.path.join(results_dir,"prepfold_ephmer_%s/prepfold_ephmer_%s_DM%3.2f"%(key,key,dm_vals[i])))
		os.chdir(os.path.join(results_dir,"prepfold_ephmer_%s/prepfold_ephmer_%s_DM%3.2f"%(key,key,dm_vals[i]))) #changing path to above initialised folder 
		...
		for p in range(int(-N*10),int(N*10),int(dt*10)):  #multiplication by 10^(-(power of 10 in the dt)), for loop does not accept the decimal values for stepsizes
			print "prepfold -noxwin -par %s/ephimeridis_%s/SAXJ1808.4-3658_%d.par ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_%d > prepfold_ephmer_o_%s"%(path,key,p,key2,dm_vals[i],key2,dm_vals[i],p,key)
			os.system("prepfold -noxwin -par %s/ephimeridis_%s/SAXJ1808.4-3658_%d.par ../../%s_DM%3.2f.dat -o %s_DM%3.2f_par_%d > prepfold_ephmer_o_%s"%(path,key,p,key2,dm_vals[i],key2,dm_vals[i],p,key))
			os.system("rm *.pfd")					
			#os.system("ls *.ps | xargs -n 1 ps2pdf") 
		os.chdir("../")
		print "ospath now is",os.getcwd()
		os.system("tar -zcvf prepfold_ephmer_%s_DM%3.2f.tar.gz prepfold_ephmer_%s_DM%3.2f"%(key,dm_vals[i],key,dm_vals[i]))   
		os.system("rm -r prepfold_ephmer_%s_DM%3.2f"%(key,dm_vals[i]))

#######################################################################################
start_time = time.time()
#
print "the array is of length",len(dm_vals)
if N_cores > Ncore_max: # safety check
        print 'Error: Asking for too many cores, reduce N_cores and try again'
        exit(-1)
index = range(1,len(dm_vals))                                            # input for parallel_process
print index
pool = mp.Pool(processes=N_cores)                                       # specifies number of parallel processes
